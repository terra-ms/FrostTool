...
async def debug_s3() -> JSONResponse:
    ...
    # 3. list bucket root
    step("list_bucket_root", lambda: [
        i.split("/")[-1] for i in fs.ls(s3_bucket, detail=False)[:20]
    ])
    ...
    def _list_mean():
        ...
        years = sorted(
            int(i.rstrip("/").split("/")[-1]) for i in items
    ...
    step("list_tmin_years", lambda: {
        "path": f"{s3_bucket}/{min_prefix}",
        "years": sorted(
            int(i.rstrip("/").split("/")[-1]) for i in
            fs.ls(f"{s3_bucket}/{min_prefix}", detail=False)
            if i.rstrip("/").split("/")[-1].isdigit()
        ),
    })
